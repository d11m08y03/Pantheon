...
const PastActivityCard = ({ title, date, description, imageUrl, category }: PastActivityCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 md:p-6 flex flex-col h-[60%] overflow-hidden">
        ...
        <h3 className="mb-2 text-lg sm:text-xl font-semibold tracking-tight text-neutral-900">{title}</h3>

        <p className="text-neutral-600 text-sm sm:text-base line-clamp-3">{description}</p>
      </div>
    </motion.div>
  );
};
